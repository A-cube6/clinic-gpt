 ...
 * - Requires an authenticated Supabase user with role = 'owner'
 *   (prefers public.staff_profiles, falls back to public.profiles).
 *
...
export async function middleware(request: NextRequest) {
  ...
  if (isLockedProdHost) {
    ...
    if (!isAdminLogin && !isAuthCallback && !isResetPassword) {
      ...
      // Your project currently uses staff_profiles for roles (owner/reception)
      // so we check that first, then fall back to profiles.
      ...
      role = (staffRow?.role as string | null) ?? null;
