...
function Modal({
  ...
  if (!open) return null;
  return (
    <section className="min-h-[72vh] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm ring-1 ring-slate-100">
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-200 bg-white/95 px-5 py-4 backdrop-blur">
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-teal-700">Details</div>
      ...
      <div className="max-h-[calc(100vh-220px)] overflow-y-auto bg-slate-50/50 p-5">{children}</div>
    </section>
  );
}
...
function RecordModal({
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/45 p-4">
      <button type="button" className="absolute inset-0" aria-label="Close editor" onClick={onClose} />
      <div className="relative max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
        ...
        <div className="max-h-[calc(90vh-64px)] overflow-y-auto bg-slate-50/50 p-5">{children}</div>
      </div>
    </div>
  );
}
...
type SectionId = "staff" | "customers" | "catalog" | "doctors" | "procedures" | "orders" | "finance" | "bookings";

export default function OwnerDashboardMasterDetail() {
  const supabase = useMemo(() => createSupabaseBrowserClient(), []);
  const syncMenuRef = useRef<HTMLDivElement | null>(null);

  const [activeSection, setActiveSection] = useState<SectionId | null>("orders");
...
// --- ORDERS (read-only for now) ---
  ...
  const filteredOrders = useMemo(
    () => orders.filter((o) => matchesDateRange(o.created_at, ordersDateFrom, ordersDateTo)),
  ...
  const loadOrders = async () => {
    ...
    const { data, error } = await supabase
      .from("orders")
      .select("id, created_at, status, subtotal_inr, shipping_inr, total_inr, customer_name, customer_phone, provider_order_id, provider_payment_id")
      .order("created_at", { ascending: false })
      .limit(50);
  ...
  const filteredBookingReqs = useMemo(
    () => bookingReqs.filter((b) => matchesDateRange(b.created_at, bookingDateFrom, bookingDateTo)),
    [bookingReqs, bookingDateFrom, bookingDateTo]
  );
...
function doctorNameFromJoin(d: DoctorJoin | undefined): string {
  ...
  const handleDownloadAll = async () => {
    ...
    downloadWorkbook("smileandcare-owner-sync.xls", [
      ...
      {
        name: "Orders",
        headers: ["id", "created_at", "status", "subtotal_inr", "shipping_inr", "total_inr", "customer_name", "customer_phone", "provider_order_id", "provider_payment_id"],
        rows: snapshot.orders.map((r) => [r.id, r.created_at ?? "", r.status ?? "", r.subtotal_inr ?? 0, r.shipping_inr ?? 0, r.total_inr ?? 0, r.customer_name ?? "", r.customer_phone ?? "", r.provider_order_id ?? "", r.provider_payment_id ?? ""]),
      },
      {
        name: "OrderItems",
  ...
  const openSection = (section: SectionId) => {
    ...
    setOpenStaff(section === "staff");
    setOpenCustomers(section === "customers");
    setOpenCatalog(section === "catalog");
    setOpenDoctors(section === "doctors");
    setOpenProcedures(section === "procedures");
    setOpenOrders(section === "orders");
    setOpenFinance(section === "finance");
    setOpenBookings(section === "bookings");
  ...
  const closeSection = () => {
    ...
    setOpenProcedures(false);
    setOpenOrders(false);
    setOpenFinance(false);
    setOpenBookings(false);
  };

  const navItems: Array<{ id: SectionId; label: string; hint: string; icon: any }> = [
    { id: "orders", label: "Orders", hint: "Recent shop orders and order items", icon: ShoppingCart },
    { id: "bookings", label: "Booking requests", hint: "Website appointment enquiries", icon: CalendarDays },
    { id: "catalog", label: "Item catalog", hint: "Products, pricing and stock", icon: Package },
    { id: "doctors", label: "Doctors", hint: "Public doctor profiles and schedule", icon: Stethoscope },
    { id: "procedures", label: "Procedures", hint: "Price list and doctor mapping", icon: ClipboardList },
    { id: "customers", label: "Customers", hint: "Customer directory", icon: UserRound },
    { id: "staff", label: "Staff", hint: "Owner and reception records", icon: Users },
    { id: "finance", label: "Assets & liabilities", hint: "Clinic financial tracking items", icon: Wallet },
  ];
  ...
  return (
    <main className="mx-auto max-w-[1480px] bg-slate-50 px-4 py-6 lg:px-6">
      <div className="flex flex-col gap-3 border-b border-slate-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div className="flex items-start gap-3">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-teal-700">Dashboard v2</div>
            <h1 className="mt-1 text-2xl font-bold text-slate-900">Owner's Workspace</h1>
            <p className="mt-1 text-sm text-slate-600">Same business logic and fields, redesigned as a side navigation with a persistent detail page.</p>
            {lastSyncAt ? <div className="mt-3 text-xs font-medium text-slate-500">Last sync: {lastSyncAt}</div> : null}
          </div>
        </div>

        <div className="flex items-center gap-2">
          <div ref={syncMenuRef} className="relative hidden md:block">
            <button
              type="button"
            ...
            {syncMenuOpen ? (
              <div className="absolute left-0 top-full z-20 mt-2 w-56 overflow-hidden rounded-2xl border border-slate-200 bg-white py-2 shadow-lg">
                <button
                  type="button"
          ...
          <button
            type="button"
            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 md:hidden"
      ...
      {mobileNavOpen ? (
        <div className="fixed inset-0 z-[90] bg-slate-950/40 md:hidden">
          <button type="button" className="absolute inset-0" aria-label="Close navigation" onClick={() => setMobileNavOpen(false)} />
          <aside className="relative h-full w-[88vw] max-w-[340px] overflow-y-auto border-r border-slate-200 bg-white p-4 shadow-xl">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm font-bold text-slate-900">Category</div>
                <div className="text-xs text-slate-500">Choose what to manage</div>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 space-y-2">
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <div className={cn("mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl", active ? "bg-teal-600 text-white" : "bg-slate-100 text-slate-700")}>
      ...
      <div className="mt-6 md:flex md:items-start md:gap-6">
        <aside
          className={cn(
            "hidden shrink-0 rounded-3xl border border-slate-200 bg-white p-3 shadow-sm md:sticky md:top-6 md:block",
            sidebarCollapsed ? "md:w-[92px]" : "md:w-[300px]"
          )}
        >
          <div className={cn("mb-3 px-2", sidebarCollapsed && "text-center")}>
            <div className="flex items-center justify-between gap-2">
              {!sidebarCollapsed ? <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-teal-700">Category</div> : <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-teal-700">Cat</div>}
              <button
                type="button"
                aria-label={sidebarCollapsed ? "Expand category" : "Collapse category"}
              ...
              >
                {sidebarCollapsed ? <PanelLeftOpen className="h-4 w-4" /> : <Menu className="h-4 w-4" />}
              </button>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <div className={cn("mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl", active ? "bg-teal-600 text-white" : "bg-slate-100 text-slate-700")}>
                    <Icon className="h-4 w-4" />
                  </div>
                  {!sidebarCollapsed ? (
        ...
        <div className="min-w-0">
          {!activeSection ? (
            <div className="flex min-h-[72vh] items-center justify-center rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
              <div>
                <div className="text-base font-bold text-slate-900">No section selected</div>
                <div className="mt-1 text-sm text-slate-600">Choose an item from the navigation to open its details view.</div>
              </div>
            </div>
          ) : null}

      <Modal title="Staff" open={openStaff} onClose={closeSection}>
        <Toolbar
          onRefresh={loadStaff}
        ...
        {staffEdit ? (
          <RecordModal title="Edit staff" open={true} onClose={() => setStaffEdit(null)}>
      ...
      <Modal title="Customers" open={openCustomers} onClose={closeSection}>
        <Toolbar
        ...
        {custEdit ? (
          <RecordModal title={custEdit.id ? "Edit customer" : "Add customer"} open={true} onClose={() => setCustEdit(null)}>
      ...
      <Modal title="Doctors" open={openDoctors} onClose={closeSection}>
        <Toolbar
        ...
        {docEdit ? (
          <RecordModal title={docEdit.id ? "Edit doctor" : "Add doctor"} open={true} onClose={() => setDocEdit(null)}>
...
downloadText(
  ...
  toCsv(rows, [
    ...
    "created_at",
        ...
        {catEdit ? (
          <RecordModal title={catEdit.id ? "Edit item" : "Add item"} open={true} onClose={() => setCatEdit(null)}>
...
<label className="block sm:col-span-4">
  ...
  />
      ...
      <Modal title="Procedures" open={openProcedures} onClose={closeSection}>
        <Toolbar
        ...
        {procEdit ? (
          <RecordModal title={procEdit.id ? "Edit procedure" : "Add procedure"} open={true} onClose={() => setProcEdit(null)}>
...
<Modal title="Orders" open={openOrders} onClose={closeSection}>
        <Toolbar
        ...
        <div className="mt-4 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-end sm:justify-between">
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block text-sm">
              <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">Created from</span>
              <input
                type="date"
            ...
            <label className="block text-sm">
              <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-slate-500">Created to</span>
              <input
                type="date"
                className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none"
                value={ordersDateTo}
          ...
          <div className="flex items-center gap-2">
            <div className="text-xs text-slate-500">Showing <span className="font-semibold text-slate-700">{filteredOrders.length}</span> of {orders.length}</div>
            <button
              type="button"
        ...
        <Table headers={["", "Created", "Customer", "Status", "Total", "Razorpay Order ID", "Razorpay Payment ID", "Order ID"]}>
            ...
            return (
              <Fragment key={o.id}>
                <tr className="border-t border-slate-200">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-slate-500">{o.provider_order_id ?? "—"}</td>
                  <td className="px-4 py-3 font-mono text-xs text-slate-500">{o.provider_payment_id ?? "—"}</td>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white p-4">
                        ...
                        <div className="mt-3 grid gap-3 md:grid-cols-2">
                          <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                            <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Razorpay Order ID</div>
                            <div className="mt-1 break-all font-mono text-xs text-slate-700">{o.provider_order_id ?? "—"}</div>
                          </div>
                          <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                            <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Razorpay Payment ID</div>
                            <div className="mt-1 break-all font-mono text-xs text-slate-700">{o.provider_payment_id ?? "—"}</div>
                          </div>
                        </div>
      ...
      <Modal title="Booking requests" open={openBookings} onClose={closeSection}>
        <Toolbar
          onRefresh={loadBookingReqs}
          onAdd={() => alert("Booking requests are created from the website booking form.")}
          addLabel="—"
          onExport={() => {
            const rows = filteredBookingReqs.map((b) => ({
              id: b.id,
        ...
        <div className="mt-4 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <div className="text-xs text-slate-500">Showing <span className="font-semibold text-slate-700">{filteredBookingReqs.length}</span> of {bookingReqs.length}</div>
            <button
              type="button"
              className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100"
              onClick={() => {
      ...
      <Modal title="Assets & liabilities" open={openFinance} onClose={closeSection}>
        <Toolbar
        ...
        {finEdit ? (
          <RecordModal title={finEdit.id ? "Edit entry" : "Add entry"} open={true} onClose={() => setFinEdit(null)}>
